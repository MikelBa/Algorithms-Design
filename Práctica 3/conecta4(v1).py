    ...
    if cont4_o>0:
        ...
        for j in range(len(nodo[6])):
            ...
            while fila<6:
                ...
                nodo_o[fila]=f2
                fila=fila+1
            nodo_x[6]=[6,6,6,6,6,6,6]
            nodo_o[6]=[6,6,6,6,6,6,6]
        for i in range(6):
            for j in range(7):
                if nodo_o[i][j]=="o":
                    cont4_o+=derecha(nodo_o,i,j,4)+arriba(nodo_o,i,j,4)+diag_der(nodo_o,i,j,4)+diag_izq(nodo_o,i,j,4)

                if nodo_x[i][j]=="x":
                    cont4_x+=derecha(nodo_x,i,j,4)+arriba(nodo_x,i,j,4)+diag_der(nodo_x,i,j,4)+diag_izq(nodo_x,i,j,4)
        return cont4_x-cont4_o
